...
* `exclude`(list\[string], 선택 사항): 제외할 파일/디렉터리에 대한 Glob 패턴(예:`["**/*.log", "temp/"]`). 다음과 같은 경우 기본 제외 항목에 추가됩니다.`use기본값Excludes`사실이다.
...
* `use기본값Excludes`(boolean, option): 기본 제외 패턴 목록을 적용할지 여부(예:`node_modules`,`.git`, 이미지/PDF 바이너리 파일이 아님). 기본값은`true`.
...
```
read_many_files(paths=["Your files or paths here."], include=["Additional files to include."], exclude=["Files to exclude."], recursive=False, use기본값Excludes=false, respect_git_ignore=true)
```
...
```
read_many_files(paths=["**/*.js"], include=["**/*.test.js", "images/**/*.jpg"], use기본값Excludes=False)
```
...
* **기본값은 다음을 제외합니다.**기본 제외 패턴(예:`node_modules`,`.git`) 그리고 사용`use기본값Excludes=False`재정의해야 하는 경우에는 신중하게 수행하세요.
